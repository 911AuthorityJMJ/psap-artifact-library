            ...
            for (const f of sec.fields) {
                if (f.kind === "scalar") idx.set(f.name, { label: f.label });
                    ...
                    for (const sf of f.fields)
                        idx.set(sf.name, { label: sf.label, loopName: f.name });
            }
        }
        return idx;
    }, [sections]);
    ...
    async function handleGenerate() {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                setGenerateError(
                    err?.error ?? `Generation failed (HTTP ${response.status}). Please try again.`,
                );
                return;
            }
                    ...
                    <div className="md:w-[42%] md:border-r border-gray-100 overflow-y-auto px-6 py-4">
                        {loadError ? (
                            ...
                            <p className="text-gray-400 text-sm">
                                This template has no fillable fields — download it directly from the
                                library instead.
                            </p>
                        ) : (
                    ...
                    <div className="px-6 py-4 border-t border-gray-100 shrink-0 flex items-center justify-between gap-3">
                        {generateError ? (
                            ...
                            <p className="text-xs text-gray-400">
                                Highlighted text shows where each field lands. Downloads a filled
                                .docx.
                            </p>
                        )}
                        <button
