...
export function deriveProfile(a: ProfileAnswers): ProfileState {
    const present = [a.itManagement, a.cybersecurityOwnership, a.operationalCapability].filter(
        isLevel,
    ) as Level[];
    ...
    const technicalLevel: Level =
        isLevel(a.callHandlingEnvironment) &&
        levelValue[a.callHandlingEnvironment] > levelValue[baseline]
...
function SelectRow({ label, value, onChange, options, invalid, muted }: RowProps) {
    return (
        <div className="flex items-center gap-4 py-2.5 border-b border-gray-100 last:border-0">
            <span className={`flex-1 text-sm ${muted ? "text-gray-400" : "text-gray-700"}`}>
                {label}
            </span>
            <select
                value={value}
    ...
    return (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <p className="text-gray-500 text-sm mb-5">
                Calibrate the artifact library to your center&apos;s capabilities. The baseline
                tracks your lowest capability — exceptions only scale up from there. All questions
                1–7 are required.
            </p>
            ...
            <div className="mb-4">
                <SelectRow
                    label="1. Who handles IT day to day?"
                    value={answers.itManagement}
                    onChange={(v) => set({ itManagement: v as Level | "" })}
                    invalid={showErrors && !isLevel(answers.itManagement)}
                    options={levelOpts([
                ...
                />
                <SelectRow
                    label="2. Who owns cybersecurity tasks?"
                    value={answers.cybersecurityOwnership}
                    onChange={(v) => set({ cybersecurityOwnership: v as Level | "" })}
                    invalid={showErrors && !isLevel(answers.cybersecurityOwnership)}
                    options={levelOpts([
                ...
                />
                <SelectRow
                    label="3. What can you operate and keep running?"
                    value={answers.operationalCapability}
                    onChange={(v) => set({ operationalCapability: v as Level | "" })}
                    invalid={showErrors && !isLevel(answers.operationalCapability)}
                    options={levelOpts([
            ...
            <div className="mb-5 px-4 py-3 bg-blue-50 border border-blue-100 rounded-lg">
                ...
                {baseline ? (
                    ...
                    <span className="text-sm text-blue-400">
                        answer questions 1–3 to set your baseline
                    </span>
                )}
            </div>
            ...
            <div className="mb-5">
                <SelectRow
                    label="4. Full-time-equivalent dispatchers?"
                    value={answers.dispatcherFteBand}
                    onChange={(v) => set({ dispatcherFteBand: v as Level | "" })}
                    invalid={showErrors && !isLevel(answers.dispatcherFteBand)}
                    options={levelOpts([
                        { value: "S", label: "1–5" },
                        { value: "M", label: "6–25" },
                        { value: "L", label: "25+" },
                ...
                />
                <SelectRow
                    label="5. Call-handling environment?"
                    value={answers.callHandlingEnvironment}
                    onChange={(v) => set({ callHandlingEnvironment: v as Level | "" })}
                    invalid={showErrors && !isLevel(answers.callHandlingEnvironment)}
                    options={levelOpts([
                ...
                />
                <SelectRow
                    label="6. Governance in place today?"
                    value={answers.governanceMaturity}
                    onChange={(v) => set({ governanceMaturity: v as Level | "" })}
                    invalid={showErrors && !isLevel(answers.governanceMaturity)}
                    options={levelOpts([
            ...
            <div className="flex items-center gap-4 py-2.5 border-b border-gray-100">
                <span className="flex-1 text-sm text-gray-700">
                    7. Access CJIS data (via IDACS)?
                </span>
                <select
                    value={answers.accessesCjis === null ? "" : answers.accessesCjis ? "yes" : "no"}
                    onChange={(e) =>
                        set({
                            accessesCjis: e.target.value === "" ? null : e.target.value === "yes",
                        })
                    }
                    className={`text-sm border rounded-lg px-3 py-2 bg-white min-w-[260px] ${
                        showErrors && answers.accessesCjis === null
            ...
            <div className="py-3">
                ...
                <div className="space-y-2 ml-1">
                    <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
                        <input
                            type="checkbox"
                            checked={answers.consolidated}
                            onChange={(e) => set({ consolidated: e.target.checked })}
                    ...
                    <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
                        <input
                            type="checkbox"
                            checked={answers.colocated}
                            onChange={(e) => set({ colocated: e.target.checked })}
                            className="w-4 h-4 rounded"
                        />
