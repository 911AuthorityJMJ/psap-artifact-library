...
function buildSubField(loopFields: { id: string }[] | undefined, subId: string): LoopSubField {
    ...
    return {
        ...
        ...((local?.options ?? global?.options)
            ? { options: local?.options ?? global?.options }
            : {}),
        ...((local?.placeholder ?? global?.placeholder)
            ? { placeholder: local?.placeholder ?? global?.placeholder }
            : {}),
    };
}
