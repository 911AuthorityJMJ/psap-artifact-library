...
function enrichQuestions(list: QuestionRow[]) {
    return list.map((q) => {
        const trace =
            traceabilityData.questionMap[q.id as keyof typeof traceabilityData.questionMap];
        const artifacts = (trace?.artifactIds ?? [])
            .map(
                (id) =>
                    traceabilityData.artifactMap[id as keyof typeof traceabilityData.artifactMap],
            )
            .filter(Boolean);
        return { ...q, artifacts };
    });
    ...
    try {
        ...
        if (!body.ok) {
            return NextResponse.json(
                { error: "File too large (250 KB max)" },
                { status: body.status },
            );
        }
            ...
            if (col0 && typeof col0 === "string" && /^\d+[A-Z]-\d+$/.test(col0)) {
                const ratingNorm =
                    typeof rating === "string"
                        ? rating.replace(/\s+/g, " ").trim().toUpperCase()
                        : "";
