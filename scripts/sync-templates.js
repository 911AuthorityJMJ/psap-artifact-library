...
function checkStem(dirLabel, filename, id, marker) {
    ...
    if (actual !== undefined && actual !== expected) {
        stemErrors.push(
            `${dirLabel}/${filename}: stem "${actual}" ≠ "${expected}" (from "${name}") — the app's links to this file will 404`,
        );
    }
}
...
function scanDir(dir, filter) {
    try {
        return readdirSync(dir).filter(
            (f) => !f.startsWith(".") && /\.(docx|xlsx)$/i.test(f) && filter(f),
        );
    } catch (err) {
...
if (unknownSizes.length) {
    console.warn(
        `Warning: examples with sizes other than ${SIZE_ORDER.join("/")} were skipped: ${unknownSizes.join(", ")}`,
    );
}
...
if (!skipConvert && formsToCompile.length) {
    ...
    for (const id of formsToCompile) {
        ...
        if (result.status !== 0) {
            console.error(
                `  Error compiling ${id} — removing partial compiled copy, leaving it untracked.`,
            );
            try {
                rmSync(dst);
            } catch {
...
if (!skipConvert) {
    ...
    for (const [id, h] of srcHash) {
        if (formIds.has(id) && existsSync(join(COMPILED_DIR, formFileMap.get(id).filename)))
            newHashes[id] = h;
    }
...
for (const id of [...allIds].sort()) {
    ...
    if (exts && exts.size > 1) {
        console.warn(
            `Warning: ${id} has examples with MIXED extensions (${[...exts].join(", ")}) — the app assumes one per artifact.`,
        );
    }
    manifest[id] = {
        form: formIds.has(id) ? formFileMap.get(id).ext : false,
        examples: exampleMap.has(id) ? [...exampleMap.get(id)].sort(sizeSort) : [],
