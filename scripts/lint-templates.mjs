...
import {
    ...
    resolveFieldForTag,
    resolveLoopById,
} from "../src/lib/field-registry.mjs";
...
for (const file of files) {
    ...
    if (leftover.length || malformed || ambiguous.length || generic.length) {
        ...
        if (leftover.length)
            console.log(
                `    ERROR  ${leftover.length} unconverted data bracket(s): ${leftover.join(", ")}`,
            );
        if (ambiguous.length)
            console.log(
                `    warn   ${ambiguous.length} ambiguous bracket(s): ${ambiguous.join(", ")}`,
            );
        if (generic.length)
            console.log(`    warn   ${generic.length} un-curated tag(s): ${generic.join(", ")}`);
    }
}

console.log(`\n── Lint summary (${files.length} docx forms) ──`);
console.log(
    `  errors:   ${errorForms} form(s)  —  ${leftoverTotal} unconverted bracket(s), ${malformedTotal} malformed loop(s)`,
);
console.log(
    `  warnings: ${ambiguousSeen.size} distinct ambiguous bracket(s), ${genericSeen.size} distinct un-curated tag(s)`,
);
